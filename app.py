...
DEFAULT_MODEL_FIELDS = {'Degree': 'B.Tech'}
...
<input type="hidden" name="Degree" value="B.Tech">
<div class="up-box">
...
MODEL_TEMPLATE = r"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Model</title><style>*{box-sizing:border-box;font-family:Arial,sans-serif}body{margin:0;background:#020617;color:#F8FAFC}.nav{background:#0F172A;border-bottom:1px solid #1E293B;padding:1rem 2rem;display:flex;gap:1.5rem}.nav a{color:#94A3B8;text-decoration:none;font-weight:700}.nav a.active{color:#6366F1}.page{max-width:1000px;margin:auto;padding:2rem}.page h1{color:#22D3EE}.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:1rem}.stat,.card{background:#0F172A;border:1px solid #1E293B;border-radius:14px;padding:1rem;margin-bottom:1rem}.stat span{display:block;color:#94A3B8}.stat b{font-size:1.5rem}.bar{display:grid;grid-template-columns:170px 1fr;gap:.7rem;align-items:center;margin:.55rem 0}.track{height:12px;background:#131C31;border-radius:999px;overflow:hidden}.fill{height:100%;background:linear-gradient(90deg,#6366F1,#22D3EE)}.matrix{display:grid;grid-template-columns:repeat(2,1fr);gap:.7rem}.cell{background:#131C31;border:1px solid #1E293B;border-radius:10px;padding:1rem;text-align:center}.note{color:#94A3B8;line-height:1.5}</style></head><body><nav class="nav"><a href="/">Predict</a><a href="/resources">Roadmap</a><a class="active" href="/model">Model</a>{% if user %}<a href="/history">History</a>{% endif %}</nav><main class="page"><h1>Model Details</h1><p style="color:#94A3B8">Transparent metrics from the training dataset and saved Random Forest model.</p>{% if metrics.error %}<div class="card">{{ metrics.error }}</div>{% else %}<div class="stats"><div class="stat"><span>Model</span><b>{{ metrics.model_name }}</b></div><div class="stat"><span>Dataset Rows</span><b>{{ metrics.dataset_size }}</b></div><div class="stat"><span>Accuracy</span><b>{{ metrics.accuracy }}%</b></div><div class="stat"><span>Precision / Recall</span><b>{{ metrics.precision }} / {{ metrics.recall }}</b></div></div><div class="card"><h3>Training Scope</h3><p class="note">Age and gender are excluded from the active model. Predictions use academic, skill, project, internship, aptitude, and backlog signals.</p></div><div class="card"><h3>Confusion Matrix</h3><div class="matrix"><div class="cell">TN<br><b>{{ metrics.confusion_matrix[0][0] }}</b></div><div class="cell">FP<br><b>{{ metrics.confusion_matrix[0][1] }}</b></div><div class="cell">FN<br><b>{{ metrics.confusion_matrix[1][0] }}</b></div><div class="cell">TP<br><b>{{ metrics.confusion_matrix[1][1] }}</b></div></div></div>{% endif %}<div class="card"><h3>Feature Importance</h3>{% for k,v in importances %}<div class="bar"><span>{{ k.replace('_',' ') }}</span><div class="track"><div class="fill" style="width:{{ v }}%"></div></div></div>{% endfor %}</div></main></body></html>"""
